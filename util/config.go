...
type configType struct {
	...
	AnsibleExtraVar string `json:"ansible_extra_var"`
}

const DEFAULT_SESSION_TIME = 15
...
func ConfigInit() {
	...
	if Config.SessionTime <= 0 {
		Config.SessionTime = DEFAULT_SESSION_TIME
	}
...
func (conf *configType) Scan() {
	...
	fmt.Print(" > session life time,default 15 minutes (default 15): ")
	fmt.Scanln(&conf.SessionTime)
	if conf.SessionTime <= 0 {
		conf.SessionTime = DEFAULT_SESSION_TIME
	}

	fmt.Print(" > Ansible Extra Variables, space for more than one (default ''): ")
	fmt.Scanln(&conf.AnsibleExtraVar)
	if len(conf.AnsibleExtraVar) == 0 {
		conf.AnsibleExtraVar = ""
	}
}
